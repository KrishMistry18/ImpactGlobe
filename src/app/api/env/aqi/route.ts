...
export async function GET() {
  try {
    ...
    const zoneIds = GLOBE_ZONES.map(z => `aqi_zone_${z.id}`)
    ...
    if (cacheIsEmpty) {
      ...
      if (needsRefresh) {
        try {
          ...
          if (points.length > 0) {
            await adminDb.collection('env_data_cache').doc(aqiZoneKey).set({
    ...
    return NextResponse.json(
      { ...aqiData, meta: { coverage: `${coverage}%`, zonesLoaded: freshZones.length, totalZones: GLOBE_ZONES.length } },
